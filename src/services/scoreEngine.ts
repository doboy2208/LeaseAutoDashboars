...
const featureScore = (car: Car, ids: string[]) => ids.filter((id) => car.features[id]?.value).length / ids.length * 100;

export const allFeatures = features;
export const defaultWeights = scoring.weights as Scores;
export const featureCount = (car: Car) => allFeatures.filter((feature) => car.features[feature.id]?.value).length;
